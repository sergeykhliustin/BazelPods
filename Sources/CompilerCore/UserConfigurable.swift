...
extension UserConfigurable {
    func apply(keyPathOperators: [String], copts: [String]) -> Self {
        ...
        for keyPathOperator in keyPathOperators {
            if keyPathOperator.contains(UserConfigurableOpt.Append.rawValue) {
                let components = keyPathOperator.components(separatedBy: UserConfigurableOpt.Append.rawValue)
                ...
                copy.override(configurableKey: key, value: value)
            }
        }
        return copy
    }
