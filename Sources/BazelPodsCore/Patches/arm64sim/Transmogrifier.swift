...
public enum Transmogrifier {
    private static func readBinary(atPath path: String, isDynamic: Bool = false) throws -> (Data, [Data], Data) {
        guard let handle = FileHandle(forReadingAtPath: path) else {
            throw "Cannot open a handle for the file at \(path). Aborting."
        }
        ...
        if header.magic != MH_MAGIC_64 || header.cputype != CPU_TYPE_ARM64 {
            throw "The file is not a correct arm64 binary. Try thinning (via lipo) or unarchiving (via ar) first."
        }
  ...
  private static func computeLoadCommandsEditor(_ loadCommandsData: [Data], isDynamic: Bool) throws -> ((Data, UInt32, UInt32) -> Data) {
    ...
    if contains_LC_VERSION_MIN_IPHONEOS == contains_LC_BUILD_VERSION {
      if contains_LC_BUILD_VERSION == true {
        throw "Bad Mach-O Object file: Both LC_VERSION_MIN_IPHONEOS and LC_BUILD_VERSION are present.\nEither one of them should be present"
      } else {
        throw "Bad Mach-O Object file: does not contain LC_VERSION_MIN_IPHONEOS or LC_BUILD_VERSION.\nEither one of them should be present"
      }
    }
  ...
  static func updateDylibFile(lc: Data, minos: UInt32, sdk: UInt32) -> Data {
    ...
    public static func processBinary(atPath path: String,
        ...
        // save back to disk
        try! reworkedData.write(to: URL(fileURLWithPath: outputPath))
    }
}
